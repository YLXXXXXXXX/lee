...
public class QualityProductController {
    @Autowired
    ...
    public ResultBean<List<Product>> listProduct(int pageindex,
        ...
        if ("".equals(keyword)) {
            list = productService.findAllByStatusGreaterThanEqual(2,pageable).getContent();
        } else {
            list = productService.findByTitleIsLikeAndStatusAfter("%" + keyword + "%",2, pageable).getContent();
        }
    ...
    public ResultBean<Integer> getTotal(HttpServletRequest request) {
        ...
        int total = 0;
        if ("".equals(keyword)) {
            total = (int) productService.findAllByStatusGreaterThanEqual(2, pageable).getTotalElements();
        } else {
            total = (int) productService.findByTitleIsLikeAndStatusAfter("%" + keyword + "%",2, pageable).getTotalElements();
        }
